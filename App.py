...
def _gh_master_path() -> str:
    return st.secrets.get("github_master_path", "rkmusic_song_master.csv")
...
def push_master_df(df: pd.DataFrame, commit_msg: str = "Update song master") -> tuple[bool, str]:
    if not _gh_secrets_ok():
        df.to_csv("rkmusic_song_master.csv", index=False, encoding="utf-8-sig")
...
def page_data_management(streaming: pd.DataFrame, master: pd.DataFrame):
    ...
    with mgmt_tab1:
        ...
        with col_ex:
            ...
            master_filename = f"rkmusic_song_master_{_date.today().strftime('%Y%m%d')}.csv"
            st.download_button(
                label="⬇️ CSVダウンロード",
                data=master_bytes,
        ...
        with col_im:
            ...
            uploaded_master = st.file_uploader(
                "rkmusic_song_master_yyyymmdd.csv を選択", type=["csv"], key="import_master"
            )
            if uploaded_master:
